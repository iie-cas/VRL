...
**注意：这有可能使你的管道通信出现问题。我只能保证第一次运行的命令一定成功。**
...
为了方便编写脚本，在moudules.tools中内置一些函数，帮助你解决一些常见问题。
...
+ aslr系列函数用于查询，更改系统的ASLR状态。如果你的脚本需要关闭ASLR，那么`aslr_off()`即可。在这种情况下，函数会检查当前状态，如果不是OFF，会弹出新的窗口请求用户输入密码更改系统设置。
